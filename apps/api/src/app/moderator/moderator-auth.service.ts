...
export class ModeratorAuthService {
  ...
  async login(data: AuthType): Promise<TokenType> {
    ...
    try {
      const user = await queryRunner.manager.createQueryBuilder(Moderator, "moderator")
        .where("moderator.email = :email", { email: data.email.trim() })
        .addSelect("moderator.password") // Critical for @Column({ select: false })
        .getOne();
      ...
      const createdToken = await this.tokenService.create(user.id, UserRole.enum.MODERATOR);
